...
class PatientFactory implements FactoryInterface
{
    ...
    public function hydrate($patient, ?array $params)
    {
        ...
        if (isset($params["datNai"])) {
            $datNai = DateTime::createFromFormat('d/m/Y', $params["datNai"]);
            if ($datNai)
                $datNai->settime(0, 0);
        ...
        if (isset($params["datDiag"])) {
            $datDiag = DateTime::createFromFormat('d/m/Y', $params["datDiag"]);
            if ($datDiag)
                $datDiag->settime(0, 0);
        ...
        if (isset($params["datLast"])) {
            $datLast = DateTime::createFromFormat('d/m/Y', $params["datLast"]);
            if ($datLast)
                $datLast->settime(0, 0);
        ...
        if (isset($params["datDeces"])) {
            $datDeces = DateTime::createFromFormat('d/m/Y', $params["datDeces"]);
            if ($datDeces)
                $datDeces->settime(0, 0);
